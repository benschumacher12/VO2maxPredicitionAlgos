...
# value for nrounds
hyper_grid <- hyper_grid[!(hyper_grid$max_depth == 0 &
                             hyper_grid$eta == 0 &
                             hyper_grid$alpha == 0), ]
...
for (i in 1:nrow(hyper_grid)){
  ...
  # Set the temporary parameters to the values ith row
  tmp_nrounds <- hyper_grid$nrounds[i]
  tmp_max_depth <- hyper_grid$max_depth[i]
  tmp_eta <- hyper_grid$eta[i]
  tmp_alpha <- hyper_grid$alpha[i]
  tmp_subsample <- hyper_grid$subsample[i]
  tmp_colsample_bytree <- hyper_grid$colsample_bytree[i]
  ...
  param <- list(booster = "gbtree",
                ...
                eta = tmp_eta,
                alpha = tmp_alpha,
                subsample = tmp_subsample,
                colsample_bytree = tmp_colsample_bytree)
  
  # Initiate the 10-fold CV xgboost training
  xgb_tree_cv <- xgb.cv(data = xgb_dat,
                        params = param,
                        nfold = 10,
                        nrounds = tmp_nrounds,
                        verbose = F,
...
# Save the model evaluation log to eval_log_individual_models
  eval_log_individual_models[[i]] <- as.data.frame(xgb_tree_cv$evaluation_log)
  
# Get the row with the minimum test RMSE value
  row_num = which.min(xgb_tree_cv$evaluation_log$test_rmse_mean)

# Create a tmp dataframe to save statistics and the nrow from the grid
  x <- as.data.frame(cbind(xgb_tree_cv$evaluation_log[row_num], i))
...
param <- list(booster = "gbtree",
              objective = "reg:squarederror",
              eval_metric = "rmse",
              max_depth = min_eval_log$max_depth[1],
              eta = min_eval_log$eta[1],
              alpha = min_eval_log$alpha[1],
              subsample = min_eval_log$subsample[1],
              colsample_bytree = min_eval_log$colsample_bytree[1])


final_xgb_tree <- xgb.cv(data = xgb_dat,
                           params = param,
                           nfold = 10,
...
final_eval_log <- as.data.frame(final_xgb_tree$evaluation_log)
final_eval_log <- final_eval_log %>% arrange(test_rmse_mean)

final_xgb_tree <- xgb.train(data = xgb_dat,
                              params = param,
                              nrounds = min_eval_log$iter[1],
                              set.seed(02082022))
